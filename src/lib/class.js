...
exports.Class = (function (Object, WeakMap) {
    ...
    var Class = function (/* optional */ Parents, members) {
        ...
        var F = function (m) {
            ...
            var iinfo = instance_info[f] = {a: ancestors, p: parents, h: priv};
        ...
        F.init = function (f, args) {
            ...
            for (var i = 0; i < Parents.length; ++i) {
                ...
                var parent = iinfo.p[Parents[i]];
                // Initialize the parent object if not already initialized.
                if (!instance_info[parent].i)
                    Parents[i].init(parent, []);
            }
        };

        var cinfo = class_info[F] = {I: id_count++};
        ...
        for (var i = 0; i < Parents.length; ++i) {
            ...
            Object.assign(ancestors, class_info[Parent].a);
        }
    ...
    Class.issubclass = function (A, B) {
        return !!(class_info.has(A)
            && class_info.has(B)
            && class_info[A].a[class_info[B].I]);
    };

    Class.isinstance = function (a, B) {
        return !!(instance_info.has(a)
            && class_info.has(B)
            && instance_info[a].a[class_info[B].I]);
    };
